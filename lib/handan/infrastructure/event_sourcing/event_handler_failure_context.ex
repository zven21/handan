defmodule Handan.EventSourcing.EventHandlerFailureContext do
  @moduledoc """
  ...
  skip: if skip is true, the event will be skipped, otherwise the process will stop (default: true)
  ...
  defmacro __using__(opts \\ []) do
    quote do
      ...
      def error(error, event, %FailureContext{context: context, metadata: metadata} = failure_context) do
        ...
        case record_failure(context) do
          %{failures: failures} when failures >= max_retries ->
            ...
            if skip do
              Logger.error("#{__MODULE__} failed to handle event, skipping after #{max_retries} retries")

              :skip
            else
              Logger.error("#{__MODULE__} failed to handle event, stopping after #{max_retries} retries")
          ...
          %{failures: failures} = context ->
            ...
            Logger.error("#{__MODULE__}  failed to handle event, retrying (#{failures}/#{max_retries})...")
