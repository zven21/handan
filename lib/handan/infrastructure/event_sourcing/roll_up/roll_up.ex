defmodule Handan.RollUp do
  ...
  import Handan.RollUp.Queries

  require Logger

  def roll_up_aggregate(stream_id, roll_up_event, stream_archive_id) do
    {:ok, pid} = Postgrex.start_link(Handan.EventStore.config())

    case Postgrex.transaction(pid, fn conn ->
           with :ok <- Handan.EventStore.delete_snapshot(stream_id, conn: conn),
  ...
  defp append_roll_up_event(conn, roll_up_event, stream_id) do
    ...
    Handan.EventStore.append_to_stream(
      stream_id,
      :any_version,
      [event_data],
      conn: conn
